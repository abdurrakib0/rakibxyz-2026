...
export default function RecommendationsClient({ initialRecommendations }: RecommendationsClientProps) {
  ...
  const totalPages = Math.max(1, Math.ceil(filteredRecs.length / ITEMS_PER_PAGE));
  const safePage = Math.min(Math.max(1, currentPage), totalPages);
  const startIndex = (safePage - 1) * ITEMS_PER_PAGE;
  const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, filteredRecs.length);
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-end gap-3 border-t border-[var(--rule)] pt-4 mt-2">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded font-mono text-[0.8125rem] transition-all duration-200 cursor-pointer flex items-center gap-1.5 font-medium bg-[var(--ink)] hover:bg-[var(--accent)] text-white"
                >
                  {isNew ? 'Save Endorsement' : 'Update Endorsement'}
                </button>
              </div>
            </form>
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          <input
            ...
            placeholder="Search endorsements by name, role, or text..."
        ...
        <div className="font-mono text-[0.75rem] text-[var(--ink-muted)] shrink-0">
          Showing <span className="font-semibold text-[var(--ink)]">{filteredRecs.length === 0 ? 0 : startIndex + 1}–{endIndex}</span> of <span className="font-semibold text-[var(--ink)]">{filteredRecs.length}</span> endorsements (10 per page)
        </div>
      </div>
      ...
      <div className="bg-[var(--surface)] border border-[var(--rule)] rounded-[var(--radius-lg)] overflow-hidden shadow-2xs">
        {filteredRecs.length === 0 ? (
          <div className="p-12 text-center font-mono text-[0.875rem] text-[var(--ink-muted)] flex flex-col items-center gap-3">
            <span>{search ? 'No endorsements match your search filter.' : 'No endorsements found.'}</span>
            {!search && (
              <button
                onClick={handleCreateNew}
